...
"""
This implements TSC over two inconsistent demos
"""
...
for i in xrange(Y.shape[0]):
	...
	Y[i, 1] = 2 * (np.tanh(x) + np.random.normal(0, .15))
...
a.addDemonstration(X)
a.addDemonstration(Y)
a.fit(normalize=True, normalizeKern="poly")
...
plt.xlim(-8, 4 * np.pi - 6+2)
plt.ylim(-5, 5)
plt.title("TSC With Polynomial Normalization")
